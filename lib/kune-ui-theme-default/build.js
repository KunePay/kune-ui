...
const buildTheme = () => {
  ...
  fileCollection.forEach((file) => {
    if (!(/(variables|colors)/).test(file)){
      ...
      // reduce that path to a sensible `dot-object` notation such as: shades.light.Heading
      const fileKey = `${((/grid\/styles\.scss$/).test(file))? '' : 'shades.'}${file.replace(/\/[^\/]+?\.scss$/, '').replace(/(^[^\/]+?)\/.+\/(.+?$)/,'$1/$2').replace(/\//g, '.')}`;

      // The `file` variable looks somehthing like: light/scss/element-components/Heading/styles.scss
      // reduce that path to a sensible distribution output path such as: dist/light/Heading.css
      const outputCSSFile = `dist/${file.replace(/\/[^\/]+?\.scss$/, '.css').replace(/(^[^\/]+?)\/.+\/(.+?\.css$)/,'$1/$2')}`;
  ...
  result = Object.assign({}, config, dot.object(result));
