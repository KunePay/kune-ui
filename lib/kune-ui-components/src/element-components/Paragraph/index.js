...
class Paragraph extends PureComponent<Props> {
  render() {
    ...
    let ParagraphComponent: any = SyncParagraph;

    if (typeof contentPromise === 'object' && typeof contentPromise.then === 'function') {
      ParagraphComponent = withAsyncChildren(ParagraphComponent, contentPromise, onContentLoaded, onContentLoadError);
    }

    return (
      <ParagraphComponent {...restProps}/>
    );
  }
}
